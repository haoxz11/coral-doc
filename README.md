...
6. **front matter**：`title` 用钉钉文档标题，`date` 用钉钉文档最后修改时间（拿不到则用当天）。
7. **文档尾部注明来源**：正文最后追加一行 `> 来源：[钉钉原文 · <标题>](<链接>)`。
8. **落盘前确认**：展示目标路径 + 转换摘要（缓存判定结果；图片 N 张：区分命中台账未重复上传 / 变更或新增已上传 / 兜底比对沿用旧链接 / 未能比对默认沿用；导不出的内容 K 处保留链接 / 不支持的元素清单）。
9. **落盘后写缓存、不做验证**：本地写入成功即完成，随后用脚本 `record-import` 把本次导入写入缓存（写失败只提示、不回滚文档）。不检查站点渲染、不回访图片链接（再次导入时为比对而回访旧链接发生在落盘前，不受此限）。
10. **清理临时文件**：过程中下载的图片、`doc read` 导出内容、比对用的中间结果全部删除。建议每次导入使用独立临时目录（如 `/tmp/dingtalk-import-<时间戳或ID>`），收尾整目录删除。`~/.coral-doc/` 缓存目录是跨会话的持久缓存，不要当临时文件删掉。
